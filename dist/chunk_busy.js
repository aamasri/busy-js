 ...
 */
"use strict";
(self["webpackChunkbusy_js"] = self["webpackChunkbusy_js"] || []).push([["busy"],{
...
/* harmony export */ });
/* harmony import */ var _busy_styl__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./busy.styl */ "./src/busy.styl");
...
const queue = []; // start loading animation

async function start(id) {
  let timeout = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 7;
  ...
  queue.push({
    ...
    expiry: expiry,
    modal: modal
  }); // allows matching id to be stopped
  ...
  if (!$busyAnimation) {
    ...
    const $body = jQuery(isInIframe ? parent.document.body : document.body); // title for debugging only
  ...
  if (modal) $modalOverlay.fadeTo('slow', 0.4);
...
function stop(id) {
  ...
  let arrayIndex; // pop specified busy id off queue
  ...
  if (arrayIndex === -1) _hideModalOverlay();
...
function reset() {
  window.setTimeout(function () {
    queue.length = 0;
    if ($busyAnimation && $busyAnimation.length) $busyAnimation.stop().fadeOut('fast');
  }, 1000);
